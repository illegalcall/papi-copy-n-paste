...
export const ENHANCED_STORAGE_QUERY_TYPES: QueryOption[] = [
  {
    ...
    useCase:
      "Perfect for: One-time checks, transaction validation, balance lookups",
  ...
  },
  {
    ...
    useCase:
      "Perfect for: Historical data, audit trails, specific block analysis",
    returnType: "Promise<T>",
  ...
  },
  {
    ...
    useCase:
      "Perfect for: Bulk operations, multiple accounts, batch processing",
    returnType: "Promise<T[]>",
  ...
  },
  {
    ...
    useCase:
      "Perfect for: Dashboards, live displays, real-time balance monitoring",
  ...
  },
  {
    ...
    useCase:
      "Perfect for: Conditional logic, filtered updates, smart contracts",
    returnType: "Observable<T>",
...
export function EnhancedQuerySelector({
  ...
  const selectedOption = ENHANCED_STORAGE_QUERY_TYPES.find(
  ...
  const OptionItem = ({ option }: { option: QueryOption }) => (
    ...
    >
      ...
      <div className="ml-5 mt-0.5 space-y-1">
        <div className="text-xs text-muted-foreground/80">{option.useCase}</div>
