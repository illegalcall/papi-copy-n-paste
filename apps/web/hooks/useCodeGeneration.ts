...
import { generateCodeSnippet, generateStorageQueryCode, generateMultiMethodCode } from '../utils/codeGenerators'
...
export function useCodeGeneration(
  ...
  const generateStorageCode = useCallback((
    ...
    try {
      ...
      console.error('Storage code generation failed:', error)
      setCode(`// Error generating code: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }, [selectedChain, selectedProvider])
  ...
  const generateMultiMethodCodeSnippet = useCallback((
    ...
    try {
      ...
      console.error('Multi-method code generation failed:', error)
      setCode(`// Error generating code: ${error instanceof Error ? error.message : 'Unknown error'}`)
      setCanRun(false)
    }
  }, [selectedChain, selectedProvider])
  ...
  const updateGeneratedCode = useCallback((
    ...
    if (methodQueue.length > 0) {
      ...
      // For storage queue, show first item as example
      const firstStorage = storageQueue[0]
      if (firstStorage) {
        generateStorageCode(
          { pallet: firstStorage.pallet, storage: firstStorage.storage },
          firstStorage.queryType,
          firstStorage.storageParams
        )
      }
  ...
  }, [generateTransactionCode, generateStorageCode, generateMultiMethodCodeSnippet])
