...
import { PalletCall, executeTransactionWithSteps, type TransactionStep } from "@workspace/core"
...
export async function executeRealTransaction(
  ...
  try {
    setConsoleOutput(prev => [...prev, `🚀 Executing ${pallet}.${call.name} transaction...`])
    ...
    setConsoleOutput(prev => [...prev, `❌ Error executing transaction: ${errorMessage}`])
  } finally {
    setIsRunning(false)
  }
}
...
export async function executeMultipleTransactions(
  ...
  for (let i = 0; i < methodQueue.length; i++) {
    ...
    try {
      ...
      if (result.success) {
        setConsoleOutput(prev => [...prev, `  ✅ Method ${methodNumber} successful! Hash: ${result.hash}`])
      } else {
...
export async function executeStorageQuery(
  ...
  try {
    ...
    if (storageQuery) {
      setConsoleOutput(prev => [...prev, `🔧 Using typed API for ${palletName}.${storageName}`])
      
      if (hasParams && paramValues.length > 0) {
        setConsoleOutput(prev => [...prev, `📝 Parameters: ${JSON.stringify(paramValues)}`])
      }
      ...
      switch (queryType) {
        case 'getValue':
          ...
          break
        case 'getValueAt':
          await executeGetValueAt(client, undefined, selectedStorage.pallet, selectedStorage.storage.name, setConsoleOutput)
          break
        case 'watchValue':
          await executeWatchValue(client, undefined, selectedStorage.pallet, selectedStorage.storage.name, setConsoleOutput)
          break
        default:
          await executeGetValue(storageQuery, paramValues, setConsoleOutput, hasParams)
      }
...
async function executeGetValue(
  ...
  try {
    const result = hasParams ? await storageQuery(...paramValues) : await storageQuery()
    const formattedResult = formatStorageResult(result)
    setConsoleOutput(prev => [...prev, `📋 Current Value: ${formattedResult}`])
...
async function executeRawStorageQuery(
  ...
  try {
    ...
    if (client._request && typeof client._request === 'function') {
      ...
      switch (queryType) {
        case 'getValue':
          await executeRawGetValue(client, palletName, storageName, setConsoleOutput)
          break
        case 'getValueAt':
          await executeGetValueAt(client, undefined, palletName, storageName, setConsoleOutput)
          break
        case 'watchValue':
          await executeWatchValue(client, undefined, palletName, storageName, setConsoleOutput)
          break
        default:
          await executeRawGetValue(client, palletName, storageName, setConsoleOutput)
      }
    } else {
      setConsoleOutput(prev => [...prev, `❌ Client does not support raw queries`])
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error'
    setConsoleOutput(prev => [...prev, `❌ Raw storage query failed: ${errorMessage}`])
  }
}
...
async function executeRawGetValue(
  ...
  try {
    ...
    if (!storageKey) {
      setConsoleOutput(prev => [...prev, `⚠️ Storage key not known for ${palletName}.${storageName}`])
      setConsoleOutput(prev => [...prev, `💡 This storage would require dynamic key generation in a full implementation`])
      return
    }
...
async function executeGetValueAt(
  ...
  try {
    if (!storageKey) {
      setConsoleOutput(prev => [...prev, `⚠️ Storage key lookup not implemented for ${palletName}.${storageName}`])
      return
    }
    ...
    setConsoleOutput(prev => [...prev, `❌ Get value at blocks failed: ${error instanceof Error ? error.message : 'Unknown error'}`])
  }
}
...
async function executeWatchValue(
  ...
  try {
    ...
    const watchInterval = setInterval(async () => {
      try {
        ...
        if (lastValue === null) {
          ...
          setConsoleOutput(prev => [...prev, `⏱️ Check ${checkCount}/5 - No change: ${decodedValue}`])
        }
        ...
        if (checkCount >= maxChecks) {
          ...
          setConsoleOutput(prev => [...prev, `✅ Finished watching ${palletName}.${storageName}`])
        }
      } catch (error) {
    ...
    setConsoleOutput(prev => [...prev, `❌ Watch setup failed: ${error instanceof Error ? error.message : 'Unknown error'}`])
  }
}