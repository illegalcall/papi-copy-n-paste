  ...
  polkadot: {
    Balances: {
      Locks: {
        ...
        actualType: 'Array<{ id: FixedSizeBinary<8>; amount: bigint; reasons: BalancesTypesReasons; }>',
        paramTypes: ['SS58String'],
        typeDefinition: 'Array<{ "id": FixedSizeBinary<8>; "amount": bigint; "reasons": BalancesTypesReasons; }>'
      },
      Account: {
        returnType: 'AccountInfo',
        actualType: '{ nonce: number; consumers: number; providers: number; sufficients: number; data: AccountData; }',
        paramTypes: ['SS58String'],
        typeDefinition: '{ "nonce": number; "consumers": number; "providers": number; "sufficients": number; "data": AccountData; }'
      },
      TotalIssuance: {
        returnType: 'bigint',
        actualType: 'bigint',
 ...
 */
export function extractActualTypes(chainKey: string, pallet: string, storageName: string): {
  returnType: string
  actualType: string
  paramTypes: string[]
  ...
  try {
    ...
    if (chainMappings && chainMappings[pallet] && chainMappings[pallet][storageName]) {
    ...
    console.warn(`Failed to extract types for ${chainKey}.${pallet}.${storageName}:`, error)
    const fallback = fallbackToInference(pallet, storageName)
    typeCache.set(cacheKey, fallback)
    return fallback
  }
 ...
 */
export function getTypeExample(actualType: string, storageName?: string): string {
  // Handle array types
  if (actualType.startsWith('Array<{')) {
    const innerMatch = actualType.match(/Array<{([^}]+)}>/s)
    if (innerMatch) {
      const fields = innerMatch[1]
      if (fields && fields.includes('id') && fields.includes('amount') && fields.includes('reasons')) {
  ...
  if (actualType.includes('FixedSizeBinary<8>') && storageName?.toLowerCase().includes('lock')) {
    return `[
  {
 ...
 */
function fallbackToInference(pallet: string, storageName: string): {
  returnType: string
  actualType: string
  paramTypes: string[]
  typeDefinition?: string
  ...
  const pattern = commonPatterns[storageName] || { returnType: 'unknown', paramTypes: [] }
 ...
 */
export function formatTypeForDisplay(typeInfo: ReturnType<typeof extractActualTypes>): string {
