...
test.describe('Multi-Network Testing', () => {
  ...
  for (const network of networks) {
    test(`should connect to ${network.name} and execute queries`, async ({ page }) => {
  ...
  test('should handle network switching between different chains', async ({ page }) => {
  ...
  test('should maintain provider selection when switching networks', async ({ page }) => {
    // Open network modal and select RPC provider for Polkadot
    const networkButton = page.locator('[data-testid="network-button"], button').first()
  ...
  test('should show appropriate providers for each network', async ({ page }) => {
    const networkButton = page.locator('[data-testid="network-button"], button').first()
    
    for (const network of networks.slice(0, 2)) { // Test first 2 networks to save time
      await networkButton.click()
      await page.waitForTimeout(1000)
      ...
      if (await networkOption.isVisible()) {
        ...
        const lightClient = page.locator('text=Light Client, [title*="Smoldot"], .text-purple-600')
        await expect(lightClient.first()).toBeVisible()
        
        // Should show RPC option
        const rpcOption = page.locator('text=RPC, [title*="RPC"], .text-blue-600')
  ...
  test('should handle connection failures gracefully', async ({ page }) => {
    ...
    if (await chopsticksProvider.isVisible()) {
      ...
      const hasStatus = await errorStatus.isVisible() || await successStatus.isVisible()
  ...
  test('should show network-specific metadata', async ({ page }) => {
    ...
    await test.step('verify Polkadot pallets', async () => {
      ...
      if (await balancesPallet.first().isVisible()) {
        ...
        const transferCall = page.locator('text=transfer, text=transfer_allow_death')
