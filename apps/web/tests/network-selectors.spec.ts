...
test.describe('Network Selectors - Two Select Dropdowns', () => {
  test('should display two separate select dropdowns for network and provider', async ({ page }) => {
    await page.goto('/')
  ...
  test('should allow selecting different networks from dropdown', async ({ page }) => {
    await page.goto('/')
    await page.waitForLoadState('domcontentloaded')
    await page.waitForTimeout(3000)
  ...
  test('should allow selecting different providers from dropdown', async ({ page }) => {
    await page.goto('/')
    await page.waitForLoadState('domcontentloaded')
    await page.waitForTimeout(3000)
    ...
    const providerSelector = providerSection.locator('[role="combobox"]').first()
    ...
    if (isProviderSelectorVisible) {
      ...
      const smoldotOption = page.locator('[role="option"]').filter({ hasText: 'Smoldot' })
