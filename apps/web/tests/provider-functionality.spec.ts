...
test.describe("Provider-Specific Functionality", () => {
  ...
  test("should test Smoldot Light Client provider", async ({ page }) => {
    ...
    const lightClientProvider = page
      .locator('text=Light Client, [title*="Smoldot"], .text-purple-600')
      .first();
    ...
    if (await lightClientProvider.isVisible()) {
      ...
      const connectionStatus = page.locator(
        ".bg-green-500, .bg-yellow-500, text=connected, text=connecting",
      );
        ...
        if (await systemPallet.isVisible()) {
          ...
          if (await chainCall.isVisible()) {
            ...
            const codeArea = page.locator(
              'pre, code, [class*="syntax"], [data-testid="generated-code"]',
            );
  ...
  test("should test Chopsticks provider", async ({ page }) => {
    ...
    if (await chopsticksProvider.isVisible()) {
      ...
      const connectionStatus = page.locator(
        ".bg-green-500, .bg-yellow-500, .bg-red-500, text=connected, text=connecting, text=error",
      );
      await expect(connectionStatus).toBeVisible();
  ...
  test("should handle provider connection errors gracefully", async ({
    ...
    if (await chopsticksProvider.isVisible()) {
      ...
      const errorIndicator = page.locator(
        ".bg-red-500, text=error, text=failed, text=disconnected",
      );
  ...
  test("should show provider-specific information", async ({ page }) => {
    ...
    const providers = page.locator(
      '[data-testid="provider-option"], .grid > div, .provider-list > *',
    );
    ...
    if (providerCount > 0) {
      ...
      for (let i = 0; i < Math.min(providerCount, 3); i++) {
        ...
        const hasText =
          (await provider
            .locator(
              "text=Light Client, text=RPC, text=Chopsticks, text=Custom",
            )
            .count()) > 0;
        if (!hasText) {
          // Or should have visual indicators (icons)
          const hasIcon =
            (await provider.locator('svg, img, [class*="icon"]').count()) > 0;
