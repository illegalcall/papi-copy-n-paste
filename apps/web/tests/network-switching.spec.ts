...
test.describe("Network Switching Functionality", () => {
  ...
  test("should display network and provider selectors in header", async ({
    page,
  }) => {
  ...
  test("should open network selector dropdown", async ({ page }) => {
    ...
    const networkSelector = page
      .locator('[role="combobox"]:has-text("Polkadot")')
      .first();
    ...
    await expect(
      page.locator('[role="option"]:has-text("Kusama")'),
    ).toBeVisible();
    await expect(
      page.locator('[role="option"]:has-text("Moonbeam")'),
    ).toBeVisible();
  });
  ...
  test("should switch between different networks", async ({ page }) => {
    ...
    const kusamaOption = page
      .locator('[role="option"]:has-text("Kusama")')
      .first();
    ...
    await expect(
      page.locator('[role="combobox"]:has-text("Kusama")'),
    ).toBeVisible();
  });
  ...
  test("should maintain network-provider combinations", async ({ page }) => {
    // Start with Polkadot
    await expect(
      page.locator('[role="combobox"]:has-text("Polkadot")').first(),
    ).toBeVisible();
  ...
  test("should show connection status indicators", async ({ page }) => {
    ...
    const statusIndicators = await Promise.race([
      page
        .locator(".bg-green-500, .bg-yellow-500, .bg-red-500")
        .first()
        .isVisible()
        .then(() => true)
        .catch(() => false),
      page
        .locator("text=Connected, text=Connecting, text=Error, text=Ready")
        .first()
        .isVisible()
        .then(() => true)
        .catch(() => false),
      page
        .locator('[data-testid*="status"], [data-testid*="connection"]')
        .first()
        .isVisible()
        .then(() => true)
        .catch(() => false),
