...
export class TemplateEngine {
  ...
  private async generatePapiConfig(): Promise<void> {
    ...
    await fs.writeJSON(
      path.join(papiDir, 'polkadot-api.json'),
      papiConfig,
      { spaces: 2 }
    )
  }
  ...
  private async processFile(filePath: string): Promise<void> {
    ...
    const textExtensions = ['.ts', '.tsx', '.js', '.jsx', '.json', '.md', '.txt', '.html', '.css', '.scss']
  ...
  private processConditionalBlocks(content: string): string {
    ...
    const chopsticksPattern = /{{#includeChopsticks}}([\s\S]*?){{\/includeChopsticks}}/g
    ...
    const examplesPattern = /{{#includeExamples}}([\s\S]*?){{\/includeExamples}}/g
