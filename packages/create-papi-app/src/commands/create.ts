...
async function gatherOptions(
  ...
  const targetPath =
    projectDirectory === "." ? process.cwd() : path.resolve(projectName);
...
async function installDependencies(options: CreateOptions): Promise<void> {
  ...
  try {
    ...
    console.warn(
      chalk.yellow("⚠️  Could not install dependencies automatically"),
    );
    console.log(
      chalk.dim(`Please run: cd ${options.projectName} && ${installCommand}`),
    );
  }
}
...
function showSuccessMessage(options: CreateOptions): void {
  ...
  console.log(
    chalk.green.bold("\n🎉 Success!"),
    `Created ${projectName} at ${options.targetPath}`,
  );
