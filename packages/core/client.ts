...
const connectionHealth = new Map<
  string,
  { lastCheck: number; isHealthy: boolean; failureCount: number }
>();
...
function updateConnectionHealth(chainKey: string, isHealthy: boolean) {
  ...
  if (isHealthy) {
    ...
    if (!isHealthyAfterFailure) {
      console.warn(
        `⚠️ Chain ${chainKey} marked as unhealthy after ${newFailureCount} consecutive failures`,
      );
    }
  }
}
...
async function createWebSocketClient(
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    for (const endpoint of endpoints) {
      try {
        console.log(
          `🔌 Trying WebSocket endpoint: ${endpoint} (attempt ${attempt + 1}/${MAX_RETRIES})`,
        );
  ...
  throw new Error(
    `All WebSocket endpoints failed for ${chainKey} after ${MAX_RETRIES} attempts`,
  );
}
...
export async function createSmoldotClient(chainKey: string) {
  ...
  const connectionPromise = (async () => {
    try {
      ...
      try {
        if (
          PUBLIC_RPC_ENDPOINTS[chainKey as keyof typeof PUBLIC_RPC_ENDPOINTS]
        ) {
          const endpoints =
            PUBLIC_RPC_ENDPOINTS[chainKey as keyof typeof PUBLIC_RPC_ENDPOINTS];
          console.log(
            `🔄 Connecting to ${chainKey} via public RPC endpoints:`,
            endpoints,
          );
          ...
          console.log(
            `Connecting to ${chainConfig.name} via WebSocket: ${chainConfig.ws}`,
          );
          const client = createClient(
        ...
        console.error(
          `❌ WebSocket connection failed for ${chainKey}:`,
          wsError,
        );
        throw wsError;
      }
      ...
      console.log(
        `🎯 Cached connection for ${chainKey} via ${connectionType} (${Date.now() - startTime}ms)`,
      );
      return result;
    } catch (error) {
...
export function useClient(chainKey: string): ClientState {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      try {
        console.log(
          `🔄 Initializing connection to ${chainKey} (attempt ${initAttempt}/${MAX_INIT_ATTEMPTS})...`,
        );
        setState((prev) => ({
          ...prev,
          status: "connecting",
        ...
        const { client, smoldot, chain, connectionType } =
          await createSmoldotClient(chainKey);

        if (!mounted) {
          console.log(
            `⚠️ Component unmounted, cleaning up ${chainKey} connection`,
          );
          if (smoldot) {
            smoldot.terminate();
          }
        ...
        if (client && client._request) {
          ...
          try {
            ...
            console.warn(
              `⚠️ Client verification failed for ${chainKey}:`,
              verifyError,
            );
        ...
        console.log(
          `✅ Successfully connected to ${chainKey} via ${connectionType}`,
        );
        setState({
          status: "ready",
          head: null,
        ...
        console.error(
          `❌ Failed to connect to ${chainKey} (attempt ${initAttempt}):`,
          errorMessage,
        );
