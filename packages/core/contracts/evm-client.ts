...
export function decodeResult(
  ...
  for (const output of outputs) {
    const word = hex.substring(offset, offset + 64);
    if (!word) break;

    if (output.type === "bool") {
      results[output.name || "0"] = BigInt("0x" + word) !== 0n;
    } else if (output.type === "address") {
      results[output.name || "0"] = "0x" + word.substring(24);
    } else if (output.type.startsWith("uint") || output.type.startsWith("int")) {
      results[output.name || "0"] = BigInt("0x" + word);
    } else {
      results[output.name || "0"] = "0x" + word;
    }
    offset += 64;
  }
...
export class EvmContractClient {
  ...
  async query(
    ...
    try {
      const result = await (this.api as any).apis.EthereumRuntimeRPCApi.call({
      ...
      const decoded = decodeResult(this.abi, methodName, result);
