...
export function getProvider(chainKey: string, providerId: string): NetworkProvider | undefined {
  const config = getNetworkConfig(chainKey)
  return config?.providers.find(provider => provider.id === providerId)
}

export function getDefaultProvider(chainKey: string): NetworkProvider | undefined {
  const config = getNetworkConfig(chainKey)
  if (!config) return undefined
  
  return config.providers.find(provider => provider.id === config.defaultProvider)
}