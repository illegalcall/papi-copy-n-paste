...
function getChainSpec(chainSpecPath: string) {
  ...
  if (!chainSpec) {
    console.warn(
      `Chain spec not found for ${chainSpecPath}, using Polkadot as fallback`,
    );
...
async function createSmoldotProvider(
  networkConfig: any,
...
function createChopsticksProvider(
  networkConfig: any,
  provider: NetworkProvider,
) {
  if (!provider.url) {
...
const connectionCache = new Map<
  string,
  { client: any; provider: NetworkProvider; timestamp: number }
>();
  ...
  try {
    ...
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error("Connection test timeout after 8s")),
          8000,
        ),
      ),
    ]);
...
export function useEnhancedClient(
  chainKey: string,
  providerId: string,
): ConnectionState {
  ...
  useEffect(() => {
    ...
    async function connect() {
      try {
        ...
        const { client, provider, connectionTime } = await createEnhancedClient(
          chainKey,
          providerId,
        );
        ...
        if (!mounted) {
          console.log(
            `⚠️ Component unmounted, cleaning up ${chainKey}:${providerId}`,
          );
          client?.destroy?.();
          return;
        }
        ...
        console.error(
          `❌ Connection failed for ${chainKey}:${providerId}:`,
          errorMessage,
        );
...
export function getSetupCommandsForProvider(
  chainKey: string,
  providerId: string,
): string {
  const networkConfig = getNetworkConfig(chainKey);
...
export function getProviderImportCode(
  chainKey: string,
  providerId: string,
): string {
  const networkConfig = getNetworkConfig(chainKey);
